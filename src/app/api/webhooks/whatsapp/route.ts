...
type WhatsAppPayload = { entry?: Array<{ changes?: Array<{ value?: WhatsAppValue }> }> };
...
function asDate(timestamp: unknown) {
...
function inboundBody(message: WhatsAppMessage) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    payload = JSON.parse(raw.toString("utf8")) as WhatsAppPayload;
  } catch {
    ...
    for (const entry of Array.isArray(payload.entry) ? payload.entry : []) {
      for (const change of Array.isArray(entry.changes) ? entry.changes : []) {
        ...
        for (const message of Array.isArray(value.messages) ? value.messages : []) {
          ...
          await admin.rpc("service_record_whatsapp_inbound", {
            p_phone_number_id: phoneNumberId,
            p_provider_message_id: String(message.id ?? ""),
            p_from_phone: String(message.from ?? ""),
            p_contact_name: String(contact?.profile?.name ?? ""),
            p_body: inboundBody(message),
            p_message_type: String(message.type ?? "unknown"),
            p_received_at: asDate(message.timestamp)
          });
        }
        ...
        for (const status of Array.isArray(value.statuses) ? value.statuses : []) {
          ...
          await admin.rpc("service_update_communication_status", {
            ...
            p_error_message: firstError?.title ?? firstError?.message ?? null,
            p_at: asDate(status.timestamp)
          });
        }
      }
    }
