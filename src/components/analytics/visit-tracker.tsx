...
export function VisitTracker({slug,tracking={}}:{slug:string;tracking?:Tracking}) {
  useEffect(()=>{
    try {
      ...
      void fetch("/api/track/visit",{method:"POST",headers:{"content-type":"application/json"},keepalive:true,body:JSON.stringify({slug,pathname:location.pathname,referrer,session_id:sessionId,...tracking})});
    } catch { /* Analytics nunca deve bloquear a experiência pública. */ }
  },[slug,tracking.utm_source,tracking.utm_medium,tracking.utm_campaign,tracking.utm_content,tracking.utm_term]);
  return null;
}
